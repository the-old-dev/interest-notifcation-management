...
public class ChangeDetectionExecutor {
	...
	public void execute(WebsiteStore websiteStore, InterestStore interestStore) {
		...
		for (Website website : websiteStore.findAll()) {
			executeChangeDetection(website, interestStore);
		}

	}

	private void executeChangeDetection(Website website, InterestStore interestStore) {
		...
		for (Subscribable subscribable : website.getSubscribables()) {
			...
			try {
				...
				detector.initialize(interestStore, website, subscribable);
				detector.run();
			} catch (InstantiationException e) {
				logger.error("Instantiation failed for the change detector of the subscribable:="+subscribable.getUrl().toExternalForm(), e);
			} catch (IllegalAccessException e) {
				logger.error("Access failed for the change detector of the subscribable:="+subscribable.getUrl().toExternalForm(), e);
			}
